...
function PwChangeEX(): JSX.Element {
  ...
  const { changePassword, errorMessage } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setToastMessage('비밀번호가 맞는지, 이전과 같진 않은지 다시 확인해주세요.');
        setShowToast(true);
      }
    } catch (error) {
      ...
      if (error instanceof Error && error.message === 'JWT 토큰이 없습니다.') {
        setToastMessage('토큰이 유효하지 않습니다. 새로운 비밀번호 변경 링크를 요청해주세요.');
      } else if (error instanceof Error && error.message.includes('401')) {
        setToastMessage('토큰이 만료되었습니다. 새로운 비밀번호 변경 링크를 요청해주세요.');
      } else {
        setToastMessage('알 수 없는 오류가 발생했습니다. 다시 시도해주시기 바랍니다 ');
      }
      setShowToast(true);
    }
  };
